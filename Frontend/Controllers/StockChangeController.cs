...
namespace Frontend.Controllers
{
    public class StockChangeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                string apiUrl = "https://localhost:5001/api/StockChange/get-stock-changes";
                var response = await _httpClient.GetAsync(apiUrl);
                if (response.IsSuccessStatusCode)
                {
                    var jsonResponse = await response.Content.ReadAsStringAsync();
                    var model = JsonConvert.DeserializeObject<List<StockChange>>(jsonResponse) ?? new List<StockChange>();
                    return View(model);
                }
                else
                {
        ...
        public async Task<IActionResult> Create([FromBody] StockChange stockChange)
        {
            ...
            var response = await _httpClient.PostAsync("https://localhost:5001/api/StockChange/create-stock-change", content);
